...
export class ConfigOverlay {
   ...
   * @param {string}   baseTopic       - e.g. "home/hsb1/pidicon-light"
  ...
  merge(base) {
    ...
    for (const d of result.devices) {
      for (const s of d.scenes || []) {
        if (!result.scenes[s]) {
          this.logger.warn(
            `[ConfigOverlay] device "${d.name}" refs unknown scene "${s}"`,
          );
        }
      }
    }
  ...
  _handleGranular(topic, payload) {
    ...
    if (rest.startsWith("device/")) {
      ...
      if (!payload) {
        ...
        try {
          patch[field] = JSON.parse(payload);
        } catch {
          this.logger.warn(
            `[ConfigOverlay] device "${name}" scenes parse failed — ignored: ${payload}`,
          );
          return;
        }
      } else {
