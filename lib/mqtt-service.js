...
export class MqttService {
  ...
  unsubscribeAll(sceneName) {
    ...
    for (const [topic] of topics) {
      if (this.client) {
        // Clean up exact-topic shared entries
        const topicEntry = this._topicEntries?.get(topic);
        if (topicEntry) {
          topicEntry.callbacks.delete(sceneName);
          if (topicEntry.callbacks.size === 0) {
        ...
        // Clean up wildcard shared entries
        const wildcardEntry = this._wildcardEntries?.get(topic);
        if (wildcardEntry) {
          wildcardEntry.callbacks.delete(sceneName);
          if (wildcardEntry.callbacks.size === 0) {
            this.client.unsubscribe(topic);
            this.client.removeListener("message", wildcardEntry.sharedHandler);
            this._wildcardEntries.delete(topic);
          }
        }
      }
      this.logger.info(
