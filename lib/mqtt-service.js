...
export class MqttService {
  ...
  subscribe(sceneName, topic, callback) {
    ...
    if (!this._topicOwners) this._topicOwners = new Map();
    ...
    const owners = this._topicOwners.get(topic) || new Set();
    const firstLogicalOwner = owners.size === 0;
    owners.add(sceneName);
    this._topicOwners.set(topic, owners);
  ...
  unsubscribeAll(sceneName) {
    ...
    for (const [topic, handler] of topics) {
      if (this.client) {
        const owners = this._topicOwners?.get(topic);
        if (owners) {
          owners.delete(sceneName);
          if (owners.size === 0) {
            this.client.unsubscribe(topic);
            this._topicOwners.delete(topic);
          }
        } else {
          this.client.unsubscribe(topic);
        }
