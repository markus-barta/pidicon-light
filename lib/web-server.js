...
<body class="min-h-screen" x-data="app()" x-init="init()" x-cloak>
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-1.5 min-h-6">
                <template x-for="scene in edits[device.name]?.scenes ?? []" :key="scene">
                  <div class="scene-chip" :title="sceneTitle(scene)" @click="openSceneSettings(device.name, scene)" style="cursor:pointer">
                    <span x-text="scenePrettyName(scene)"></span>
                    <button @click.stop="removeScene(device.name, scene)" title="Remove" style="display:flex">
  ...
  <div x-show="showSceneSettings" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4"
    ...
    <div class="glass-card rounded-2xl w-full max-w-5xl max-h-[92vh] p-6 flex flex-col gap-5 overflow-hidden" @click.stop>
      ...
      <div class="flex gap-2 pt-1">
        <button @click="clearSceneOverlay()" class="btn btn-sm btn-ghost flex-1" :disabled="!mqttConnected || !currentSceneSettings">Clear Overlay</button>
        <button @click="applySceneOverlay()" class="btn btn-sm btn-ghost flex-1" :disabled="!mqttConnected || !currentSceneSettings">Set Overlay</button>
        <button @click="saveSceneSettings()" class="btn btn-sm btn-primary flex-1" :disabled="!currentSceneSettings">Save Settings</button>
        <button @click="cloneScene()" class="btn btn-sm btn-outline flex-1" :disabled="!currentSceneSettings">Clone Scene</button>
      </div>
    </div>
  </div>
