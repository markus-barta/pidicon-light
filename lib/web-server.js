...
<head>
  ...
  <style>
    ...
    .scene-actions-stack {
      ...
      bottom: 0;
      padding-top: 12px;
      background: linear-gradient(180deg, rgba(9,11,18,0) 0%, rgba(9,11,18,0.92) 18%, rgba(9,11,18,0.98) 100%);
    }
    ...
    .scene-actions-card-main {
      display: flex;
      align-items: flex-start;
      gap: 12px;
      min-width: 0;
    }
    .scene-actions-card-text {
      min-width: 0;
      flex: 1;
    }
    .scene-actions-card-title {
      font-size: 13px;
      font-weight: 600;
      color: #fff;
      line-height: 1.2;
    }
    .scene-actions-card-help {
      font-size: 11px;
      ...
      <div class="scene-actions-stack pt-1">
        <div class="scene-actions-stack-primary">
          <div class="scene-actions-card">
            <div class="scene-actions-card-main">
              <span class="scene-help-badge" title="Save writes these values to config.json for this device and scene.">i</span>
              <div class="scene-actions-card-text">
                <div class="scene-actions-card-title">Save</div>
                <div class="scene-actions-card-help">Save these values for this device and this scene key. If multiple devices use the same scene key, their own saved values can still differ.</div>
              </div>
            </div>
            <button @click="saveSceneSettings()" class="btn btn-sm btn-primary" :disabled="!currentSceneSettings">Save</button>
          </div>
        </div>
        <details>
          <summary class="scene-advanced-toggle">
            <span>Advanced</span>
            <span style="display:flex" x-html="icon('chevron-right', 14)"></span>
          </summary>
          <div class="scene-advanced-panel">
            <div class="scene-actions-card">
              <div class="scene-actions-card-main">
                <span class="scene-help-badge" title="Overlay sends a temporary live override over MQTT. It does not replace saved settings.">i</span>
                <div class="scene-actions-card-text">
                  <div class="scene-actions-card-title">Preview Live</div>
                  <div class="scene-actions-card-help">Try the current values live without saving them permanently.</div>
                </div>
              </div>
              <button @click="applySceneOverlay()" class="btn btn-sm btn-ghost" :disabled="!mqttConnected || !currentSceneSettings">Preview Live</button>
            </div>
            <div class="scene-actions-card">
              <div class="scene-actions-card-main">
                <span class="scene-help-badge" title="Remove the temporary MQTT override and go back to the saved scene settings.">i</span>
                <div class="scene-actions-card-text">
                  <div class="scene-actions-card-title">Clear Live Preview</div>
                  <div class="scene-actions-card-help">Remove the temporary live override and go back to the saved settings.</div>
                </div>
              </div>
              <button @click="clearSceneOverlay()" class="btn btn-sm btn-ghost" :disabled="!mqttConnected || !currentSceneSettings">Clear Live Preview</button>
            </div>
            <div style="border-top:1px solid rgba(255,255,255,0.08);padding-top:14px">
              <div class="section-label">Make A Separate Scene Copy</div>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <div class="section-label mb-1.5">New Scene Key</div>
                  <input class="field font-mono" placeholder="home-kids" x-model="sceneCloneForm.targetSceneKey">
                </div>
                <div>
                  <div class="section-label mb-1.5">Name Override</div>
                ...
                <div class="md:col-span-2">
                  <label class="flex items-center gap-3 h-[42px] px-3 rounded-lg" style="border:1px solid rgba(255,255,255,0.08);background:rgba(255,255,255,0.03)">
                    ...
                    <span class="text-sm text-white/80">Replace the current scene key with a generated copy first backed up on disk.</span>
                  </label>
                </div>
              </div>
              <div class="scene-actions-card" style="margin-top:12px">
                <div class="scene-actions-card-main">
                  <span class="scene-help-badge" title="Create a generated copy so you can customize this scene separately from the original file.">i</span>
                  <div class="scene-actions-card-text">
                    <div class="scene-actions-card-title">Create Separate Copy</div>
                  <div class="scene-actions-card-help">Advanced: create a new scene file and scene key so one device can diverge from the original scene.</div>
                </div>
                </div>
                <button @click="cloneScene()" class="btn btn-sm btn-outline" :disabled="!currentSceneSettings">Create Separate Copy</button>
              </div>
            </div>
          </div>
        </details>
    ...
    function app() {
      return {
        ...
        openSceneSettings(deviceName, sceneName) {
          ...
          this._loadSceneSettingsForm(deviceName, sceneName);
          this.sceneCloneForm = {
        ...
        _loadSceneSettingsForm(deviceName, sceneName) {
          ...
          const schema = this.sceneMeta?.[sceneName]?.settingsSchema || {};
          for (const [key, field] of Object.entries(schema)) {
            if (field.type === 'json') {
